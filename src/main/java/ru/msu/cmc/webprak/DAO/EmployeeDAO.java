...
public class EmployeeDAO extends CommonDAO<Employee, Integer> {
    ...
    public List<Instance> instanceById(Integer employeeId) {
        try (Session session = sessionFactory.openSession()) {
            String hql = "FROM Instance o WHERE o.employee.id = :employeeId";

            Query<Instance> query = session.createQuery(hql, Instance.class);
